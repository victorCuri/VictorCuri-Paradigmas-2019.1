...
public class MatchesAdapter extends RecyclerView.Adapter<MatchesViewHolders> {
    ...
    @Override
    public void onBindViewHolder(@NonNull MatchesViewHolders holder, int i) {

        holder.mMatchId.setText(matchesList.get(i).getUserId());
        holder.mMatchName.setText(matchesList.get(i).getName());
        if(!matchesList.get(i).getProfileImageUrl().equals("default")){
            Glide.with(context).load(matchesList.get(i).getProfileImageUrl()).into(holder.mMatchImage);
        }
    }
